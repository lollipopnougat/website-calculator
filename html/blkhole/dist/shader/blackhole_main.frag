...
void adiskColor(vec3 pos, inout vec3 color, inout float alpha) {
  ...
  float noise = 1.0;
  float noiseLOD = floor(adiskNoiseLOD);
  for (int i = 0; i < 12; i++) {
    if (float(i) >= noiseLOD) break;
    ...
    noise *= 0.5 * snoise(sphericalCoord * scale * adiskNoiseScale) + 0.5;
    
    if (floor(mod(float(i), 2.0)) < 0.5) {
...
vec3 traceColor(vec3 pos, vec3 dir) {
  ...
  color += textureCube(galaxy, dir).rgb * alpha;
  return color;
}
...
void main() {
  ...
  gl_FragColor = vec4(traceColor(pos, dir), 1.0);
}
